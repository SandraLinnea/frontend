...
export default function BookingsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await apiFetch("/api/booking", { credentials: "include" });
...
import { apiGet } from "@/utils/fetch"; // ⬅️ enda importändringen
import { useEffect, useMemo, useState } from "react";
...
export default function BookingsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // ⬇️ apiFetch("/api/booking") → apiGet("/booking")
        const j = await apiGet<{ data: Booking[] }>("/booking");
        setData(Array.isArray(j) ? j : j.data ?? []);
      } catch (e) {
        setMsg((e as Error).message);
      } finally {
