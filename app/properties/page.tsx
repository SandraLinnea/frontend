...
export default function PropertiesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await apiFetch("/property?limit=50&offset=0", {
          credentials: "include",
        });
...
import { useEffect, useState } from "react";
import Link from "next/link";
import { apiGet } from "@/utils/fetch"; // ⬅️ ändrad import
...
export default function PropertiesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // ⬇️ apiFetch → apiGet
        const json = await apiGet<{ data: Property[]; error?: string }>(
          "/property?limit=50&offset=0"
        );
        setData(json.data ?? []);
      } catch (err) {
        setError("Ett fel uppstod när boenden skulle hämtas");
